...
#------------------------------------------------------------------------------------------------------------------------------------------------------------------
# IMPORTING FLU DESEQ2 FILE

input_Fludf = read.csv(paste0(in_path, 'deseq_FLU_TAG_padj.csv'), row.names = 1)
knitr::kable(head(input_Fludf))

#--------------------------------------------------------------------------------------------------------------------------------------------------------------------

#for Fludf
# Check for NA values in P_VALUE column and handle them ===================================================
if(any(is.na(input_Fludf$FDR_p))) {
  # Assign NA p-values to 1
  input_Fludf$FDR_p[is.na(input_Fludf$FDR_p)] <- 1
}


# Ensure input data has required columns
required_cols <- c("Gene_symbol", "logFC", "FDR_p")
if(!all(required_cols %in% colnames(input_Fludf))) {
...
filtered_data_flu <-  input_Fludf%>%
  filter(FDR_p< 0.1 & (logFC > 1 | logFC < -1))

write.csv(filtered_data_flu, paste0(out_path,  'Flu_pathway_TAG_filtered_padjvalue_0.1&log1_outputdf.csv'), row.names = FALSE)

#----------------------------------------------------------------------------------------------------------------------------------------------------------

# Run pathfindR to get bubble plot showing enriched pathways

Flu_output <- run_pathfindR(filtered_data_flu, output_dir=out_path,
                            gene_sets = "mmu_KEGG",
...
write.csv(Flu_output, paste0(out_path,  'Flu_pathway_TAGadjpvalue_0.1 log1&-1_outputdf.csv'), row.names = FALSE)

#------------------------------------------------------------------------------------------------------------------------------------------------------------

#for flu termgene heatmap

term_gene_heatmap(result_df = Flu_output, use_description = TRUE)

#-------------------------------------------------------------------------------------------------------------------------------------------------------------

#for FLU term_gene graph

term_gene_graph(result_df = Flu_output, use_description = TRUE)

#-------------------------------------------------------------------------------------------------------------------------------------------------------------

#for FLU Upset plot

UpSet_plot(Flu_output, use_description = TRUE)
